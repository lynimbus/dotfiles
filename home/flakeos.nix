...

{
  ...
  # settings 留空 → 用内置默认配置（自带顶栏 [bar.main]）；想托管 config.toml
  # 就在这里填 attrset（会经 `noctalia config validate` 校验后写入
  # ~/.config/noctalia/config.toml）。旧 Arch 的 config.toml 不在 dotfiles 仓里。
  programs.noctalia = {
    enable = true;
  };
