...

{
  ...
  home.packages = with pkgs; [
    ...
    ayugram-desktop
    qq
    # 微信：UOS 商店 .deb 装进 buildFHSEnv。选它而不是 pkgs.wechat（腾讯官方
    # AppImage）的理由：版本更新（4.1.1.7 vs 4.1.1.4）；启动脚本按 XMODIFIERS
    # 自动设 QT_IM_MODULE/GTK_IM_MODULE（fcitx-qt5 是编译进微信二进制的，设了
    # 变量就能用，不依赖外部插件）；desktop 补了 StartupWMClass=wechat，托盘和
    # 窗口匹配才正常。两者都是静态链接 Qt 的 X11-only 程序（NEEDED 里只有
    # xcb/X11，无 wayland），必须走 XWayland —— niri 26.04 内建 XWayland，
    # DISPLAY 已就绪，不需要额外起 xwayland-satellite。
    wechat-uos
  ...
  # niri 声明式配置（niri-flake homeModules.config 托管，构建期用
  # programs.niri.package＝pkgs.niri-glass 跑 `niri validate` 后写入
  ...
  programs.niri.settings = {
    ...
    "window-rules" = [
      ...
      # 微信（wechat-uos，X11 进 XWayland，StartupWMClass=wechat）。
      # 不透明 + 关毛玻璃：与 chromium/QQ 同理，自绘界面叠半透明会发灰。
      # rule: wechat-opaque
      {
      ...
      # 图片查看、转发选人等小窗走浮动，别把主列挤变形。主窗与登录窗
      # 标题同为「微信」，无法区分 → 不匹配，两者都保持平铺。
      # rule: wechat-dialogs-float
      {
        matches = [
          {
            "app-id" = "^wechat$";
  ...
  # 静态资源由 Home Manager 链接到 store；auth.json / sessions 等运行时状态留在本机。
  home.file = {
